...
const InativeTasksTable = () => {
    ...
    useEffect(() => {
      ...
      const handleDeleteForever = (taskId, tokenUser) => {
        ...
        NotificationManager.info("Are you sure you want to delete this task?", "Confirm", 7000, () => {
  ...
  return (
    ...
    {task.priority === 300 ? <FcHighPriority /> : task.priority === 200 ? <FcMediumPriority /> : task.priority === 100 ? <FcLowPriority /> : ""}
   
</td>
<td>
<button className='edit_button' onClick={() => handleRestore(tokenUser, task.id)}><MdOutlineRestore /></button>
                            <button className='delete_button' onClick={() => handleDeleteForever(task.id, tokenUser)}><MdDeleteForever /></button>
</td>
